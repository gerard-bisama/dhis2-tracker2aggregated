...
function setupApp () {
  ...
  app.get('/convertevent2adx', (req, res) => {
	...
	var currentPageEvent=0;
	var globalEventsList=[];
	getAdxPager(appConfig.adxEvent.orgId,appConfig.adxEvent.stageId,appConfig.adxEvent.startDate,appConfig.adxEvent.endDate,
		function(adxPager)
		{
			...
			getStageEventsByPeriodSkipPaging(_startDate,appConfig.adxEvent.endDate,
				_incrementDate,appConfig.adxEvent.stageId,appConfig.adxEvent.orgId,globalEventsList,function(eventsList){
			var listADxPayLoad=generateAdxKPPREVFromEvents(eventsList);
			//return;
...
function generateAdxKPPREVFromEvents(eventsList)
{
	...
	for(var index=0;index<listEventKPPrev.length;index++)
	{
		...
		var aggregatedValue=getAggregatedDataElementValues(oEvent);
		reformatedList.push({
			id:oEvent.event,
	...
	var kpPrevDateDefinition=getKpPrevDataDefinition();
	//console.log(result);
	/*
	var listTemp=[];
	for(var index=0;index<result.length;index++)
	{
		if(result[index].eventDate=="2019-10-14")
		{
		...
		var xmlObject=[{adx:[{_attr:{xmlns:'urn:ihe:qrph:adx:2015','xmlns:xsi':'http://www.w3.org/2001/XMLSchema-instance',
			...
			{group:[{_attr:{orgUnit:listData[index].orgUnit,period:listData[index].eventDate+"/P1D",completeDate:currentZFormatDate,dataSet:dataSetId}},
				{dataValue:[{_attr:{dataElement:dataElementId,
					...
					Partenaireexecution:partenaireExecutionId,value:listData[index].nbre}}]}
...
function getAggregatedDataElementValues(event)
{
	var aggregatedValue={
		sexe:"",
		categorieKeyPop:"",
	...
	var aggregateDataElements=appConfig.adxEvent.aggregateDataElements;
...
function generateKPPrevDataElements(eventsList){
	...
	var kpPrevDataDefinition=getKpPrevDataDefinition();
